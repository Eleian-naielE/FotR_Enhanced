...
function IA_Spawn_Land(dummy_name, script_name, marker_name_in, faction_name_in)
	...
	if marker_name_in then
		...
		local pos_s, pos_e = string.find(script_name,"Add_InstantActionUnit_Land_")
		marker_name = string.sub(script_name,pos_e+1)
	end
	...
	Spawn_Unit(spawn_object_type, Find_First_Object("INSTANTACTION_LAND_MARKER_"..string.upper(marker_name)), Find_Player(faction_name))
