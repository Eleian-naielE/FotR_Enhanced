return {
	Era_1 = {
		{
			{"CEC_Light_Cruiser", 5},
			{"Class_C_Frigate", 5},
			{"Class_C_Support", 5},
			{"Republic_Trooper_Company", 5},
			{"Republic_Heavy_Trooper_Company", 5},
			{"Republic_Navy_Trooper_Company", 5},
		},
		{
			{"PDF_DHC", 3},
			{"DHC_Carrier", 2},
			{"Republic_Jedi_Knight_Company", 3, nil, DisablingEvent = "ORDER_66"},
			{"Republic_Gian_Company", 4},
		},
		{
			{"Antarian_Ranger_Company", 5, nil, DisablingEvent = "ORDER_66"},
			{"Special_Tactics_Trooper_Company", 5},
			{"Senate_Commando_Company", 5},
			{"Republic_VAAT_Company", 2},
			{"Republic_A4_Juggernaut_Company", 2},
			{"Starbolt", 4},
		}
	},
	Era_2 = {
		{
			{"CEC_Light_Cruiser", 5},
			{"Clone_Special_Ops_Company", 5},
			{"Clone_Flame_Trooper_Company", 5},
			{"Clone_Galactic_Marine_Company", 5},
			{"Special_Tactics_Trooper_Company", 5},
			{"Republic_Navy_Trooper_Company", 5},
			{"Republic_Trooper_Company", 5},
		},
		{
			{"Charger_C70", 5},
			{"DHC_Carrier", 2},
			{"Antarian_Ranger_Company", 5, nil, DisablingEvent = "ORDER_66"},
			{"Senate_Commando_Company", 5},
			{"Clone_Jumptrooper_Phase_One_Company", 3},
			{"Republic_A5RX_Company", 2},
			{"Republic_VAAT_Company", 2},
			{"SPHA_T_Company", 2}, --FotR_Enhanced
		},
		{
			{"RX200_Falchion_Company", 2},
			--{"Republic_A6_Juggernaut_Company", 1},
			{"Republic_A6_Prototype_Company", 1}, --FotR_Enhanced
			{"Starbolt", 4},
		}
	},
	Era_3 = {
		{
			{"Customs_Corvette", 5},
			{"CEC_Light_Cruiser", 5},
			{"Clone_Special_Ops_Company", 5},
			{"Clone_Flame_Trooper_Company", 5},
			{"Clone_Galactic_Marine_Company", 5},
			{"Special_Tactics_Trooper_Company", 5},
			{"Republic_Navy_Trooper_Company", 5},
			{"Republic_Trooper_Company", 5},
		},
		{
			{"Charger_C70", 5},
			{"DHC_Carrier", 2},
			{"Antarian_Ranger_Company", 5, nil, ["DisablingEvent"] = "ORDER_66"},
			{"Senate_Commando_Company", 5},
			{"Clone_Jumptrooper_Phase_One_Company", 3},
			{"Republic_A5RX_Company", 2},
			{"Republic_VAAT_Company", 2},
		},
		{
			{"RX200_Falchion_Company", 2},
			--{"Republic_A6_Juggernaut_Company", 1},
			{"Republic_A6_Prototype_Company", 1}, --FotR_Enhanced
			{"Starbolt", 4},
		}
	},
	Era_4 = {
		{
			{"Customs_Corvette", 5},
			{"CEC_Light_Cruiser", 5},
			{"Clone_Scout_Trooper_Company", 5},
			{"Clone_Airborne_Trooper_Company", 5},
			{"Clone_Galactic_Marine_Company", 5},
			{"Clone_Flame_Trooper_Company", 5},			
			{"Special_Tactics_Trooper_Company", 5},
			{"Republic_Navy_Trooper_Company", 5},
			{"Republic_Trooper_Company", 5},
		},
		{
			{"Charger_C70", 5},
			{"Antarian_Ranger_Company", 5, nil, ["DisablingEvent"] = "ORDER_66"},
			{"Senate_Commando_Company", 5},
			{"Clone_Jumptrooper_Phase_Two_Company", 3},
			{"Clone_Blaze_Trooper_Company", 5},			
			{"Republic_A5RX_Company", 2},
			{"Republic_VAAT_Company", 2},
		},
		{
			{"RX200_Falchion_Company", 2},
			{"Republic_A6_Juggernaut_Company", 1},
			{"Gladiator_I", 3},
			{"Starbolt", 4},
		}
	},
	Era_5 = {
		{
			{"Customs_Corvette", 5},
			{"CEC_Light_Cruiser", 5},
			{"Clone_Scout_Trooper_Company", 5},
			{"Clone_Airborne_Trooper_Company", 5},
			{"Clone_Galactic_Marine_Company", 5},
			{"Clone_Flame_Trooper_Company", 5},			
			{"Special_Tactics_Trooper_Company", 5},
			{"Republic_Navy_Trooper_Company", 5},
			{"Republic_Trooper_Company", 5},
		},
		{
			{"Charger_C70", 5},
			{"Antarian_Ranger_Company", 5, nil, ["DisablingEvent"] = "ORDER_66"},
			{"Senate_Commando_Company", 5},
			{"Clone_Jumptrooper_Phase_Two_Company", 3},
			{"Clone_Blaze_Trooper_Company", 5},			
			{"Republic_A5RX_Company", 2},
			{"Republic_VAAT_Company", 2},
		},
		{
			{"RX200_Falchion_Company", 2},
			{"Republic_A6_Juggernaut_Company", 1},
			{"Gladiator_I", 3},
			{"Starbolt", 4},
			{"Imperial_DHC", 3},
		}
	},
	PDF = {
		{
			{"LAC", 5},
			{"Consular_Refit", 5},
			{"Class_C_Frigate", 5},
			{"Customs_Corvette", 5},
			{"Storm_Cloud_Car_Company", 3},
			{"Light_Mercenary_Company", 5},
			{"Mercenary_Company", 5},
		},
		{
			{"Mandalorian_Soldier_Company", 4},
			{"Mandalorian_Commando_Company", 3},
			{"Republic_Gian_Company", 4},
			{"Ailon_Nova_Guard_Company", 3},
			{"Sun_Guard_Company", 5},
		},
		{
			{"Kuari_Princess_Liner", 3},
			{"Invincible_Cruiser", 1},
			{"Procurator_Battlecruiser", 1},
			{"Elite_Mercenary_Company", 3},
		}
	}
}
