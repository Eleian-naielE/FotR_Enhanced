return {
	TRADE_FEDERATION = {
		{
			{"MAF_Company", 2},
			{"MTT_Company", 2},
			{"Neimoidian_Guard_Company", 5},
		},
		{
			{"Defoliator_Company", 2},
			{"Sabaoth_Frigate", 5},
			{"Sabaoth_Frigate_Spy", 5},
			{"Sabaoth_Hex_Deployer", 4},
			{"Seismic_Tank_Company", 1},
		},
		{
			{"Lucrehulk_Auxiliary", 1},
			{"Sabaoth_Destroyer", 2},
		}
	},
	TECHNO_UNION = {
		{
			{"Marauder_Cruiser", 5},
			{"Geonosian_Cruiser", 5},
			{"Skakoan_Combat_Engineer_Company", 5},
		},
		{
			{"Super_Tank_Company", 2},
		},
		{
			{"Storm_Fleet_Destroyer", 2},
			{"Recusant_Dreadnought", 1},
		}		
	},
	COMMERCE_GUILD = {
		{
			{"GAT_Company", 3},
			{"CA_Artillery_Company", 2},
			{"Persuader_Assault_Company", 3},
		},
		{
			{"Kaloth_Battlecruiser", 5},
			{"CIS_PDF_DHC", 3},
			{"Seismic_Tank_Company", 1}
		},
		{
			{"Providence_Dreadnought", 1},
			{"Providence_Carrier", 2},			
		},
	},
	IGBC = {
		{
			{"Interceptor_III_Frigate", 5},
			{"Action_VI_Support", 5},
			{"Hardcell_Tender", 5},
		},
		{
			{"Munificent_Subfaction", 3},
			{"Munificent_Tender", 3},
			{"Munificent_C3", 3},
			{"Lucrehulk_Core_Destroyer", 1},
		},
		{
			{"Munificent_Heavy_Cruiser", 2},
			{"Lucrehulk_Auxiliary_Control", 1},
			{"Storm_Fleet_Destroyer", 2},
		}
	},
	CIS = {
		{
			{"B1_Droid_Marine_Company", 5},
			{"Nimbus_Commando_Company", 5},
			{"B2_RP_Droid_Company", 5},
		},
		{
			{"CIS_Defoliator_Company", 2},
			{"CIS_Super_Tank_Company", 2},
			{"Storm_Fleet_Destroyer", 2},
		},
		{	-- We want A6 to have a low chance, so other rewards get extra bump
			{"Lucrehulk_Bulk_Cruiser", 1},
			{"Lucrehulk_Bulk_Cruiser", 1},
			{"Lucrehulk_Bulk_Cruiser", 1},
			{"Lucrehulk_Bulk_Cruiser", 1},
			{"Protodeka_Company", 1},
			{"Protodeka_Company", 1},
			{"Protodeka_Company", 1},
			{"Protodeka_Company", 1},
			{"CIS_A6_Juggernaut_Company", 1},
		}
	},
	PDF = {
		{
			{"CEC_Light_Cruiser", 5},
			{"Storm_Cloud_Car_Company", 3},
			{"Customs_Corvette", 5},
			{"Light_Mercenary_Company", 5},
			{"Mercenary_Company", 5},
		},
		{
			{"Mandalorian_Soldier_Company", 4},
			{"Mandalorian_Commando_Company", 3},
			{"Neutron_Star_Mercenary", 3},
			{"Ailon_Nova_Guard_Company", 4},
			{"Sun_Guard_Company", 3},
		},
		{
			{"Space_ARC_Cruiser", 2},
			{"Lucrehulk_Auxiliary_Control", 1},
			{"Elite_Mercenary_Company", 3},
			{"A5_Juggernaut_Company", 2},
		}
	}
}